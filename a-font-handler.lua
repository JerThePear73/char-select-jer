...
function djui_hud_print_text(message, x, y, scaleX, scaleY)
    ...
    if customFont then
        ...
        y = y + currFont.offset
        scaleX = scaleX*currFont.scale
        scaleY = scaleY*currFont.scale
        for i = 1, #message do
            ...
            if letter and letter ~= " " then
                ...
                local scaleWidth = scaleX*(currFont.info[letter].height/currFont.info[letter].width)
                djui_hud_render_texture_tile(currFont.spritesheet,
                x + ((currFont.info[letter].xoffset or 0)*scaleX) + math.random(-textShake*100, textShake*100)*0.01 + math.sin((HudAnimTimer+i*2)*textWaveSpeed*0.1)*textWaveX,
                y + ((currFont.info[letter].yoffset or 0)*scaleY) + math.random(-textShake*100, textShake*100)*0.01 + math.cos((HudAnimTimer+i*2)*textWaveSpeed*0.1)*textWaveY,
                scaleWidth, scaleY,
            ...
            x = x + (currFont.info[letter].width + currFont.spacing)*scaleX
        end
    else
        djui_classic_hud_print_text(message, x, y, scaleX, scaleY)
    end
end
...
function djui_hud_print_text_interpolated(message, prevX, prevY, prevScaleX, prevScaleY, x, y, scaleX, scaleY)
    if customFont then
        ...
        y = y + currFont.offset
        prevScaleX = prevScaleX*currFont.scale
        prevScaleY = prevScaleY*currFont.scale
        scaleX = scaleX*currFont.scale
        scaleY = scaleX*currFont.scale
        for i = 1, #message do
            local letter = message[i]
            if letter and letter ~= " " then
                if currFont.info[letter] == nil then
                ...
                local prevScaleWidth = prevScaleX*(currFont.info[letter].height/currFont.info[letter].width)
                local scaleWidth = scaleX*(currFont.info[letter].height/currFont.info[letter].width)
                local xOffset = ((currFont.info[letter].xoffset or 0)*scaleX) + math.random(-textShake*100, textShake*100)*0.01 + math.sin((HudAnimTimer+i*2)*textWaveSpeed*0.1)*textWaveX
                local yOffset = ((currFont.info[letter].yoffset or 0)*scaleY) + math.random(-textShake*100, textShake*100)*0.01 + math.cos((HudAnimTimer+i*2)*textWaveSpeed*0.1)*textWaveY 
                djui_hud_render_texture_tile_interpolated(currFont.spritesheet,
                prevX + xOffset,
                prevY + yOffset,
                prevScaleWidth, prevScaleY,
                x + xOffset,
                y + yOffset,
                scaleWidth, scaleY,
                currFont.info[letter].x,
            ...
            x = x + (currFont.info[letter].width + currFont.spacing)*scaleX
            prevX = prevX + (currFont.info[letter].width + currFont.spacing)*prevScaleX
        end
    else
        djui_classic_hud_print_text_interpolated(message, prevX, prevY, prevScaleX, prevScaleY, x, y, scaleX, scaleY)
    end
end
...
function djui_hud_measure_text(message)
    if customFont then
        ...
        for i = 1, #message do
            ...
            y = math.max(y, currFont.info[letter].height*scale)
        end
        return x, y
    else
...
FONT_CHARACTERISTIC = djui_hud_add_font(get_texture_info("char_select_font_characteristic"), fontdataCharacteristic, -5, 0, "X", 0.5)